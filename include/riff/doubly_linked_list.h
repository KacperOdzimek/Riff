...
RIFF_API(void) RIFF_INST(dlist_zero, INSTANCE)(dlist(INSTANCE)* tar) {
    tar->priv_size  = 0;
    tar->priv_first = NULL;
    tar->priv_last  = NULL;
}
...
RIFF_API(void) RIFF_INST(dlist_destroy, INSTANCE)(dlist(INSTANCE)* tar) {
...
RIFF_API(size_t) RIFF_INST(dlist_size, INSTANCE)(const dlist(INSTANCE)* tar) {
...
RIFF_API(dlist_node(INSTANCE)*) RIFF_INST(dlist_first, INSTANCE)(const dlist(INSTANCE)* tar) {
...
RIFF_API(dlist_node(INSTANCE)*) RIFF_INST(dlist_last, INSTANCE)(const dlist(INSTANCE)* tar) {
...
RIFF_API(dlist_node(INSTANCE)*) RIFF_INST(dlist_next, INSTANCE)(dlist_node(INSTANCE)* n) {
...
RIFF_API(dlist_node(INSTANCE)*) RIFF_INST(dlist_prev, INSTANCE)(dlist_node(INSTANCE)* n) {
...
RIFF_API(STORED*) RIFF_INST(dlist_access, INSTANCE)(dlist_node(INSTANCE)* n) {
    return &n->priv_obj;
}
...
RIFF_API(const STORED*) RIFF_INST(dlist_const_access, INSTANCE)(dlist_node(INSTANCE)* n) {
    return &n->priv_obj;
}

#ifndef dlist_const_access
...
RIFF_API(dlist_node(INSTANCE)*) RIFF_INST(dlist_push_before, INSTANCE)(dlist(INSTANCE)* tar, dlist_node(INSTANCE)* before, STORED value) {
...
RIFF_API(dlist_node(INSTANCE)*) RIFF_INST(dlist_push_after, INSTANCE)(dlist(INSTANCE)* tar, dlist_node(INSTANCE)* after, STORED value) {
    dlist_node(INSTANCE)* new_node = (dlist_node(INSTANCE)*)RIFF_ALLOC(sizeof(dlist_node(INSTANCE)));
    if (!new_node) return NULL;
...
RIFF_API(void) RIFF_INST(dlist_pop, INSTANCE)(dlist(INSTANCE)* tar, dlist_node(INSTANCE)* n, STORED* out) {
    // get rid of object
...
RIFF_API(void) RIFF_INST(dlist_clear, INSTANCE)(dlist(INSTANCE)* tar) {
